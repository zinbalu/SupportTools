...
// let submitButton = document.getElementById('office-to-submit'); //placeholder
// if(submitButton) submitButton.addEventListener('click', function (event) {
// 	if(event.type === 'click') {
// 		const account = storage.get('account').data;
// 		let responseArea = jQuery(".office-to-response")[0]; //placeholder
...
// 								 '</div>';
// 		$.ajax({
// 			url: "http://"+account.server+"/api.php?target=ApiSupportTools.changeOfficeTo", //placeholder
// 			dataType: "json",
// 			type: "POST",
// 			data: JSON.stringify({
...
let buttonOfficeTo = document.getElementById('office-to-submit'); //placeholder
...
function callAPI(method, submitButtonID, responseAreaID, params){
	const storage = require('electron-json-storage-sync');
	const account = storage.get('account').data;
	// let responseArea = jQuery("."+responseAreaClassName)[0]; //placeholder
	let responseArea = document.getElementById(responseAreaID); //placeholder
	let submitButton = document.getElementById(submitButtonID); //placeholder
	let oldValue = submitButton.innerHTML;
	submitButton.disabled = true;
	submitButton.innerHTML = '<div style="white-space: nowrap; margin-left: -21px; margin-top: -7px;">' +
		'<img src="css/loading.gif" style="vertical-align: middle;"/>' +
		...
		'</div>';
	$.ajax({
		url: "http://"+account.server+"/api.php?target=ApiSupportTools."+method, //placeholder
		dataType: "json",
		type: "POST",
		data: JSON.stringify(params),//placeholder
		contentType: "application/json; charset=utf-8",
		xhrFields: {
			crossDomain: true,
			withCredentials: true
